...
//                $resultSet[]=$keys['product_id'];
    ...
    public function edit_cart($cart_id, $qid, $oqty) {
        $update_query = 'UPDATE `' . self::CART_QUOTE_ITEM_TABLE . '` SET `product_qty`=' . $oqty . ' WHERE `id`=' . $cart_id . ' AND `quote_id`=' . $qid;
        $this->db->query($update_query);
    }
    ...
    public function fetchQuoteItemByCartId($cart_id) {
        $this->db->select('*');
        $this->db->from(self::CART_QUOTE_ITEM_TABLE);
