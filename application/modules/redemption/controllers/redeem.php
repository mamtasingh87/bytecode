...
//                        $userModel->make_user_amount_deduction($id,$total_amount);
                        $msg = '<p class="success">' . $cond['data']['name'] . ' was added to cart. <a href="{{ site_url }}/redemption/redeem/showcart">click here</a> to checkout</p>';
                        $success = TRUE;
                    } else {
    ...
    public function editcartqty() {
        ...
        try {
            if ($this->input->get('pid')) {
                ...
                if ($res['type']) {
                    $cartModel = $this->load->model('redemption/cart_model');
                    $cart_item_details = $cartModel->fetchQuoteItemByCartId($cart_id);
...
//                  $amount = ($price_rate * $ord_qty);
                    $amount = ($cart_item_details['product_price'] * $ord_qty);
                    if ($user_model->check_user_amount_point($u_id, $amount)) {
                        $cartModel->edit_cart($cart_id, $quoteId['quote'], $ord_qty);
