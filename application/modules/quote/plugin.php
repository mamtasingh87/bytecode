...
class Quote_plugin extends Plugin {
    ...
    function do_upload_quote_doc($files) {
        $this->load->library('upload');
       $arr=array();
        $cpt = count($_FILES['request_document']['name']);
        for ($i = 0; $i < $cpt; $i++) {
            $_FILES['request_document']['name'] = $files['request_document']['name'][$i];
            $_FILES['request_document']['type'] = $files['request_document']['type'][$i];
            $_FILES['request_document']['tmp_name'] = $files['request_document']['tmp_name'][$i];
            $_FILES['request_document']['error'] = $files['request_document']['error'][$i];
            $_FILES['request_document']['size'] = $files['request_document']['size'][$i];
            $this->upload->initialize($this->set_upload_quote_options());
            $this->upload->do_upload('request_document');
        ...
        $this->request_document=$arr;
    }
    function do_upload_binder_doc($files) {
        $this->load->library('upload');
       $arr=array();
        $cpt = count($_FILES['requested_document']['name']);
        for ($i = 0; $i < $cpt; $i++) {
            $_FILES['requested_document']['name'] = $files['requested_document']['name'][$i];
            $_FILES['requested_document']['type'] = $files['requested_document']['type'][$i];
            $_FILES['requested_document']['tmp_name'] = $files['requested_document']['tmp_name'][$i];
            $_FILES['requested_document']['error'] = $files['requested_document']['error'][$i];
            $_FILES['requested_document']['size'] = $files['requested_document']['size'][$i];
            $this->upload->initialize($this->set_upload_binder_options());
            $this->upload->do_upload('requested_document');
            $error = $this->upload->display_errors();
        ...
        $this->requested_document=$arr;
    }
    private function set_upload_binder_options() {
    ...
    private function set_upload_quote_options() {
        //upload an image options
        $config = array();
...
//        }
        if ($this->input->post('submit')) {
            ...
            $this->form_validation->set_rules('client_email', 'Email', 'required|trim|xss_clean|valid_email|max_length[100]');
            $this->form_validation->set_rules('client_phone', 'Phone No', 'required|format_phone|max_length[20]');
           ...
           $this->request_document = '';
            if (@$_FILES['request_document']['name'] != "") {
               $this->do_upload_quote_doc($_FILES);
            }
    ...
    private function _process_quote_request_form($form_data, $file='', $name = '') {
        ...
        if (isset($form_data['client_dob']) && $form_data['client_dob']) {
            $date = DateTime::createFromFormat('m-d-Y', $form_data['client_dob']);
            $form_data['client_dob'] = $date->format('Y-m-d');
        }
        if (isset($form_data['effective_date']) && $form_data['effective_date']) {
            $test_date = date('m-d-Y', strtotime($form_data['effective_date']));
            $unix_date = date("01-01-1970");
            if ($test_date - $unix_date == 0) {
                $date_arr = explode("-", $form_data['effective_date']);
                if (strlen(strval($date_arr[0])) == 4) {
                    $date = DateTime::createFromFormat('Y-d-m', $form_data['effective_date']);
                } else {
            $date = DateTime::createFromFormat('m-d-Y', $form_data['effective_date']);
                }
            $form_data['effective_date'] = $date->format('Y-m-d');
            } else {
                $date = date('Y-m-d', strtotime($form_data['effective_date']));
                $form_data['effective_date'] = $date;
        }
//            print_r($form_data['effective_date']);exit;
        }
...
//            $scoreData['user_id'] = $this->secure->get_user_session()->id;
            $scoreData['user_id'] = $form_data['requested_by'];
//            $scoreData['earn_point'] = 5;
            $club = $userModel->getUserById($scoreData['user_id']);
            $club = $club->club;
            ...
            $scoreData['earn_point'] = ($club == 'bronze-club' ? $data['Settings']->bronze_club_quote_point->value : ($club == 'silver-club' ? $data['Settings']->silver_club_quote_point->value : ($club == 'gold-club' ? $data['Settings']->gold_club_quote_point->value : '0')));

            $scoreData['earn_amount'] = $this->settings->amount_earned_trivia_correct_answer;
            $userModel->update_user_score($scoreData);
            $LogModel->saveQuoteRequestLog($scoreData);
            /**
             * Points while quote save 
             */
            if(!empty($file)){
            $result=$this->quote_request_model->Upload_Quote_Document($file,$insert_id);
            }
...
//        }
        if ($this->input->post('submit')) {
            ...
            if (@$_FILES['requested_document']['name'] != "") {
               $this->do_upload_binder_doc($_FILES);
            }
            $this->form_validation->set_error_delimiters('<span class="error">', '</span>');
    ...
    public function _process_binder_request_form($form_data, $file = '', $name = '', $email = '', $phone_no = '', $zohoData = array()) {
        ...
        if (isset($form_data['closing_date']) && $form_data['closing_date']) {
            $date = DateTime::createFromFormat('m-d-Y', $form_data['closing_date']);
            $form_data['closing_date'] = $date->format('Y-m-d');
        }
...
//            $scoreData['earn_point'] = 5;
            ...
            $scoreData['earn_point'] = ($club == 'bronze-club' ? $data['Settings']->bronze_club_binder_point->value : ($club == 'silver-club' ? $data['Settings']->silver_club_binder_point->value : ($club == 'gold-club' ? $data['Settings']->gold_club_binder_point->value : '0')));
//              print_r($scoreData['earn_point']);exit;
            $scoreData['earn_amount'] = $this->settings->amount_earned_trivia_correct_answer;
            $userModel->update_user_score($scoreData);
            $LogModel->saveBinderRequestLog($scoreData);
            /**
             * Points while quote save 
             */
            if(!empty($file)){
            $result=$this->binder_request_model->Upload_Binder_Document($file,$insert_id);
            }
