...
class Quote extends Admin_Controller {
    ...
    function changestatus() {
        ...
        if ($this->input->post('selected')) {
            ...
            $formData = $requestsModel->getDetailsById($ids);
            if ($change_to == 1) {
                $this->_send_quote_approval($formData); //($form_data);
            } else {
                $this->_send_quote_disapproval($formData); //($form_data);
            }
        } else {
    ...
    private function _send_quote_approval($formData) {//($form_data)
        ...
        send_format_template(24, $params, TRUE);
    }

    private function _send_quote_disapproval($formData) {//($form_data)
        if (isset($formData['request_document']) && $formData['request_document']) {
            $request_file = '<a href="' . site_url('users/account/download/quote/' . $formData['request_document']) . '">Click to download' . '</a>';
        } else {
            $request_file = 'N/A';
    ...
    public function change_binder_status() {
        ...
        if ($ids) {
            ...
            $formData = $binderModel->getBinderByid($ids);
            if ($status == 2) {
                $this->_send_binder_approval($formData); //($form_data);
            } else {
                $this->_send_binder_disapproval($formData); //($form_data);
            }
            $this->session->set_flashdata('message', '<p class="success">Status successfully changed.</p>');
        } else {
    ...
    private function _send_binder_approval($formData) {//($form_data)
    ...
    private function _send_binder_disapproval($formData) {//($form_data)
        if (isset($formData['requested_document']) && $formData['requested_document']) {
            $request_file = '<a href="' . site_url('users/account/download/binder/' . $formData['requested_document']) . '">Click to download' . '</a>';
        } else {
            $request_file = 'N/A';
        ...
        send_format_template(26, $params, TRUE);
    }
