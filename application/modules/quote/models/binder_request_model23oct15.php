...
class Binder_request_model extends DataMapper {
    ...
    public function getAllStatus() {
        return array(0 => '', self::STATUS_PENDING => 'Pending', self::STATUS_APPROVED => 'Approved', self::STATUS_DECLINE => 'Disapproved');
    }

    public function getAllStatusForGrid() {
        return array(0 => 'Select Status', self::STATUS_APPROVED => 'Approved', self::STATUS_DECLINE => 'Disapproved');
    }

    public function getAllStatusForFilter() {
        return array('' => 'Status', self::STATUS_PENDING => 'Pending', self::STATUS_APPROVED => 'Approved', self::STATUS_DECLINE => 'Disapproved');
    }
    ...
    function SaveBinderToZoho($form_data, $file = '') {
        ...
        $xmlData = '<SalesOrders>      
<row no="1">
<FL val="Borrower Name">' . $form_data['borrower_name'] . '</FL>
<FL val="Borrower Email">' . $form_data['borrower_email'] . '</FL>
<FL val="Borrower Phone">' . $form_data['borrower_phone'] . '</FL>
<FL val="Loan Number">' . $form_data['loan_number'] . '</FL>
<FL val="Closing Date">' . $form_data['closing_date'] . '</FL>
<FL val="Quote Amount to be Bound">' . $form_data['premium_quote'] . '</FL>
<FL val="Mortgage Clause">' . $form_data['mortgage_clause'] . '</FL>
...
//print_r($result);
    ...
    public function get_all_binder($sort = '', $order = '', $limit = '', $start = '', $filter = array()) {
        ...
        $this->db->order_by(($sort) ? $sort : 'b.id', ($order) ? $order : 'desc');
    ...
    public function changeStatus($ids, $status) {
        ...
        if ($ids) {
            ...
            $status = ($status == 1 ? 'Pending' : ($status == 2 ? 'Approved' : 'Disapproved'));
            $this->ChangeBinderStatusToZoho($status, $recordID);
        }
    }

    function ChangeBinderStatusToZoho($status = '', $recordID = '') {
        $ch = curl_init('https://crm.zoho.com/crm/private/xml/SalesOrders/updateRecords?');
        ...
        $xmlData = '<SalesOrders>      
<row no="1">
<FL val="Current Status">' . $status . '</FL>
</row>
</SalesOrders>';
...
//  exit;
    ...
    public function getBinderByid($id = NULL) {
        if ($id) {
