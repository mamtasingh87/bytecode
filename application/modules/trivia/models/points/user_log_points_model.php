...
class User_log_points_model extends DataMapper {
    ...
    public function saveQuoteRequestLog($data = array()){
        $this->db->insert($this->table, array('credit_to' => $data['user_id'], 'points' => $data['earn_point'], 'points_credit_on' => date('Y-m-d'), 'status' => 2, 'type' => 4));
    }
    public function saveBinderRequestLog($data = array()){
        $this->db->insert($this->table, array('credit_to' => $data['user_id'], 'points' => $data['earn_point'], 'points_credit_on' => date('Y-m-d'), 'status' => 2, 'type' => 5));
    }
