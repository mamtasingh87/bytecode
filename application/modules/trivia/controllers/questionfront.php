    ...
    function questionday() {
        ...
        if ($this->input->post()) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if ($status == 1) {
                    $this->session->set_flashdata('question_message', '<p class="success">Congratulations! Your answer is correct. You have earned ' . $formData['earn_point'] . ' point.</p>');
                } else {
    ...
    public function referfriend() {
        if ($this->input->post()) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $stringToCode = $this->secure->get_user_session()->email;
                ...
                $params = array(
                    ...
                    '{{sender_name}}' => $this->secure->get_user_session()->first_name.' '.$this->secure->get_user_session()->last_name,
                    '{{refer_url}}' => $referUrl,
                );
